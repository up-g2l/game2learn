 ...
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 * This file must be included in the jsp pages for practice
 */
...
$(function(){
    ...
    $.getJSON( "/g2l/MCTestTaking?"+qString, function( data ) {
        jsonArrQ = eval(data);
        $('#numOfQ').text(" ("+jsonArrQ.length+") ");        
        loadQ(0);
    ...
    $("#aBtnSaveAns").click(function(){
        if($('#divAnsBox').css('visibility')=='hidden'){$('#divAnsBox').css('visibility','visible');}
           else $('#divAnsBox').css('visibility','hidden');
    });
   ...
   // $('#divAnsBox').css("visibility","hidden");
...
function loadQ(nxt){
    ...
    if(qIndex > jsonArrQ.length-1){//If one the last question
        $('#pIns').text("You are on the last question now.");
        $('#insMessage').show();
        setTimeout(function(){
            $('#insMessage').hide();
        },2000);
    ...
    if(jsonArrQ[qIndex].figName!=="0.jpg"){$('#imgBox').show('slow');}
   ...
   // $('#flashAnsBox').css('visibility','hidden');//Only for Flash Practice
    ...
    $('input[type=radio]').attr('checked',false).next().css("border","1px solid #FFFFFF");//Default answer box style
    ...
    MathJax.Hub.Queue(['Typeset',MathJax.Hub,'divPracQBox']);
    return true;
}
...
function getTestInfo(){
    ...
    if($('#testId').val()==""){
        ...
        $.post("../MCTestTaking?activity=testInfo",
        { 
            testId: $('#testId').val()
        },
        function(data,status){    
            if(status=='success')  {
                ...
                alert("Failure in fetching data");
...
function submitTest(){
    ...
    if(!confirm("Are you sure of submitting the test?")){ return;}
    $('#btnSubmitTest').prop('disabled', true);
    
    for(var i=0;i<jsonArrQ.length;i++){
        currQ = jsonArrQ[i];  
        ansList = ansList + currQ.qId+":"+currQ.choice+";";
        if(currQ.ansCorr == currQ.choice){score = score + 1;}
    }
    console.log("Data sent to server = "+ansList);
